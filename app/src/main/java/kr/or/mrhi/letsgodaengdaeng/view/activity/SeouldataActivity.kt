                ...
                override fun onResponse(call: Call<Library>, response: Response<Library>) {
                    ...
                    data.let {
                        for (loadData in it!!.LOCALDATA_020301.row) {
                            if (loadData.TRDSTATEGBN == "01" && !loadData.X.isNullOrEmpty() && !loadData.Y.isNullOrEmpty()) {
                                ...
                                val veterinary =
                                    Veterinary(code, name, address, phone, longitude, latitude)
                ...
                override fun onResponse(
                    ...
                    data?.let {
                        for (loadData in it.SeoulGilWalkCourse.row) {
                            ...
                            val longitude = wgsPt.x
                            val latitude = wgsPt.y
                            val seoulGil = kr.or.mrhi.letsgodaengdaeng.dataClass.SeoulGil(
                                name, local, distance, time, detailCourse,
    ...
    fun animalPhoto() {
        animalPhotoservice.getLibrarys(PHOTO_API_KEY, PHOTO_LIMIT)
            .enqueue(object : Callback<AnimalPhotoLibrary> {
                override fun onResponse(
                    call: Call<AnimalPhotoLibrary>,
                    response: Response<AnimalPhotoLibrary>
                ) {
                    val data = response.body()
                    data?.let {
                        var number = 0
